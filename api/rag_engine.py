...
class RAGEngine:
    ...
    def get_context(self, question: str, tenant_id: str, top_k: int = 5) -> RetrievalContext:
        """Retrieve relevant context chunks for a question without generating an answer."""
        start_time = time.time()
        scored_chunks = self.search_and_rank(question, tenant_id, top_k)
        ...
        for index, (chunk, score) in enumerate(scored_chunks):
            ...
            section_title = chunk.section or f"Section {index + 1}"
            context_sections.append(f"{chunk.title} - {section_title}:\n{chunk.text}")
